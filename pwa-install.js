...
(function() {
    ...
    const PWAInstall = {
        init() {
            console.log('[PWA] Initializing PWA Install System...');
            console.log('[PWA] User Agent:', navigator.userAgent);
            console.log('[PWA] Is Standalone:', Utils.isStandalone());
            console.log('[PWA] Is iOS:', Utils.isIOS());
            console.log('[PWA] Is Mobile:', Utils.isMobile());
            ...
            const dismissed = this.wasRecentlyDismissed();
            console.log('[PWA] Was recently dismissed:', dismissed);
            
            if (dismissed) {
                console.log('[PWA] ❌ Install prompt was recently dismissed');
                // TEMPORAL: Mostrar de todos modos para debugging
                console.log('[PWA] 🔧 DEBUG MODE: Showing anyway...');
            }

            // iOS: Mostrar instrucciones manuales
            if (Utils.isIOS()) {
                console.log('[PWA] 📱 iOS detected - will show instructions');
                setTimeout(() => this.showIOSInstructions(), CONFIG.showDelay);
                return;
            }

            // Android/Desktop: Esperar evento beforeinstallprompt
            console.log('[PWA] 🤖 Android/Desktop detected - waiting for beforeinstallprompt event');
            this.setupInstallPrompt();
            
            // TEMPORAL: Agregar botón de prueba manual
            this.addDebugButton();
        },
        
        addDebugButton() {
            // Crear botón flotante de debug
            const debugBtn = document.createElement('button');
            debugBtn.id = 'pwaDebugBtn';
            debugBtn.innerHTML = '<i class="fas fa-download"></i>';
            ...
            debugBtn.addEventListener('click', () => {
                ...
                if (deferredPrompt) {
                    ...
                    alert('El evento beforeinstallprompt aún no se ha disparado. Revisa la consola para más detalles.');
                }
            });
            ...
            console.log('[PWA] 🔧 DEBUG: Added manual trigger button (orange, bottom right)');
        },
        ...
        setupInstallPrompt() {
            ...
            window.addEventListener('beforeinstallprompt', (e) => {
                ...
                e.preventDefault();
                console.log('[PWA] Default prompt prevented');
                ...
                console.log(`[PWA] Will show custom popup in ${CONFIG.showDelay}ms`);
                setTimeout(() => {
                    console.log('[PWA] Showing custom install popup now...');
                    this.showInstallPopup();
                }, CONFIG.showDelay);
            });
            ...
            window.addEventListener('appinstalled', () => {
                ...
                // Remover botón de debug
                const debugBtn = document.getElementById('pwaDebugBtn');
                if (debugBtn) {
                    debugBtn.remove();
                }
            });
