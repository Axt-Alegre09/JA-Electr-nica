/**
 * JA ELECTRÓNICA - Main Application Script v2.1
 */
(function() {
    'use strict';

    const CONFIG = {
        storageKeys: { theme: 'ja_theme', cart: 'ja_cart', newsletter: 'ja_newsletter_shown' },
        selectors: {
            themeToggle: '#themeToggle', hamburgerBtn: '#hamburgerBtn', filtersSidebar: '#filtersSidebar',
            overlay: '#overlay', userBtn: '#userBtn', userDropdown: '#userDropdown', mainSearch: '#mainSearch',
            sortSelect: '#sortSelect', productsGrid: '#productsGrid', productCards: '.product-card-bristol',
            filterLinks: '.filter-link', filterCheckbox: '#withDiscount', cartBtn: '#cartBtn',
            cartModal: '#cartModal', cartCloseBtn: '#cartCloseBtn', newsletterPopup: '#newsletterPopup',
            brandsShowcase: '.brands-showcase', brandsLink: '.main-nav a[href="#brands"]'
        },
        delays: { search: 300, sidebarClose: 300, newsletterShow: 2000, notification: 3000 }
    };
    ...
    const Utils = {
        ...
        formatNumber(num) { return new Intl.NumberFormat('es-PY').format(num); },
        storage: {
    ...
    const Theme = {
        ...
        toggle() {
            ...
            setTimeout(() => { if (toggle) toggle.classList.remove('rotating'); }, 500);
        }
    };
    ...
    const Sidebar = {
        ...
        init() {
            ...
            this.elements.hamburger.addEventListener('click', (e) => { e.stopPropagation(); this.open(); });
            if (this.elements.overlay) this.elements.overlay.addEventListener('click', () => this.close());
            document.addEventListener('keydown', (e) => {
                if (e.key === 'Escape' && this.elements.sidebar.classList.contains('active')) this.close();
            });
        ...
        initTouchGestures() {
            ...
            document.addEventListener('touchend', (e) => {
                ...
                if (diff > 100 && touchStartX < 50 && !this.elements.sidebar.classList.contains('active')) this.open();
                if (diff < -100 && this.elements.sidebar.classList.contains('active')) this.close();
            }, { passive: true });
        }
    };
    ...
    const Filters = {
        ...
        init() { this.bindFilterLinks(); this.bindCheckboxFilter(); },
        bindFilterLinks() {
            Utils.$$(CONFIG.selectors.filterLinks).forEach(link => {
                link.addEventListener('click', (e) => {
                    ...
                    if (brand) { this.applyFilter('brand', brand); this.updateTitle('brand', brand); }
                    else if (special) { this.applyFilter('special', special); this.updateTitle('special', special); }
                    else if (gender) { this.applyFilter('gender', gender); this.updateTitle('gender', gender); }
                    if (window.innerWidth < 992) setTimeout(() => Sidebar.close(), CONFIG.delays.sidebarClose);
                });
            });
        },
        bindCheckboxFilter() {
            ...
            checkbox.addEventListener('change', () => {
                if (checkbox.checked) { this.applyFilter('discount', true); this.updateTitle('special', 'sale'); }
                else this.clearAll();
            });
        },
        applyFilter(type, value) {
            ...
            products.forEach(product => {
                let show = true;
                switch (type) {
                    case 'brand': show = product.dataset.brand === value; break;
                    case 'gender': show = product.dataset.gender === value; break;
                    case 'special':
                        ...
                        break;
                    case 'discount': if (value) show = Utils.$('.product-badge-bristol.sale', product) !== null; break;
                }
        ...
        updateTitle(type, value) {
            ...
            titleEl.innerHTML = `<i class="${titleData.icon}" aria-hidden="true"></i>${titleData.name}`;
        ...
        clearAll() {
            ...
            Utils.$$(CONFIG.selectors.productCards).forEach(product => { product.style.display = ''; });
            this.updateTitle('default', 'default');
            this.updateProductCount();
        },
        updateProductCount() {
            const visible = Utils.$$(CONFIG.selectors.productCards).filter(p => p.style.display !== 'none').length;
    ...
    const Search = {
        init() {
            ...
            searchInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') this.perform(searchInput.value); });
        },
        perform(query) {
            const searchTerm = query.toLowerCase().trim();
            const products = Utils.$$(CONFIG.selectors.productCards);
            products.forEach(product => {
                if (!searchTerm) { product.style.display = ''; return; }
                ...
                const matches = brand.includes(searchTerm) || title.includes(searchTerm) || category.includes(searchTerm);
    ...
    const Cart = {
        ...
        bindEvents() {
            ...
            if (cartBtn && cartModal) cartBtn.addEventListener('click', () => this.openModal());
            if (closeBtn) closeBtn.addEventListener('click', () => this.closeModal());
            if (cartModal) cartModal.addEventListener('click', (e) => { if (e.target === cartModal) this.closeModal(); });
        ...
        addItem(card, button) {
            ...
            else this.items.push({ id, title, brand, price, image, quantity: 1 });
            this.save();
            this.updateBadge();
            this.render();
        ...
        showAddFeedback(button) {
            ...
            setTimeout(() => { button.innerHTML = original; button.style.background = ''; button.disabled = false; }, 2000);
        },
        updateQuantity(id, change) {
            const item = this.items.find(i => i.id === id);
            if (item) { item.quantity = Math.max(1, item.quantity + change); this.save(); this.updateBadge(); this.render(); }
        },
        removeItem(id) {
            this.items = this.items.filter(item => item.id !== id);
            this.save(); this.updateBadge(); this.render();
        },
        updateBadge() {
            const total = this.items.reduce((sum, item) => sum + item.quantity, 0);
            Utils.$$('.cart-btn .badge, .cart-count-badge').forEach(badge => {
                badge.textContent = total;
                if (badge.classList.contains('badge')) badge.style.display = total > 0 ? '' : 'none';
            });
        },
        render() {
            ...
            if (this.items.length === 0) {
                body.innerHTML = `<div class="cart-empty"><i class="fas fa-shopping-cart"></i><p>Tu carrito está vacío</p><a href="index.html" class="btn-primary btn-block">Seguir Comprando</a></div>`;
                    ...
                    <div class="cart-item-info">
                        ...
                        <div class="cart-item-actions">
                            <div class="quantity-control">
                                <button class="quantity-btn" onclick="Cart.updateQuantity('${item.id}', -1)"><i class="fas fa-minus"></i></button>
                                <input type="number" class="quantity-input" value="${item.quantity}" min="1" onchange="Cart.setQuantity('${item.id}', this.value)">
                                <button class="quantity-btn" onclick="Cart.updateQuantity('${item.id}', 1)"><i class="fas fa-plus"></i></button>
                            </div>
                            <button class="cart-item-remove" onclick="Cart.removeItem('${item.id}')"><i class="fas fa-trash"></i></button>
                        </div>
                    </div>
                </div>
            `).join('');
        ...
        setQuantity(id, value) {
            ...
            const item = this.items.find(i => i.id === id);
            if (item) { item.quantity = quantity; this.save(); this.updateBadge(); this.render(); }
        },
        save() { Utils.storage.set(CONFIG.storageKeys.cart, this.items); },
        clear() { this.items = []; this.save(); this.updateBadge(); this.render(); }
    };
    window.Cart = Cart;
    ...
    const BrandsShowcase = {
        init() {
            ...
            link.addEventListener('mouseenter', () => { if (!isMobile()) show(); });
            link.addEventListener('mouseleave', () => {
                if (!isMobile()) {
                    hideTimeout = setTimeout(() => { if (!showcase.matches(':hover')) hide(); }, 200);
                }
            });
            showcase.addEventListener('mouseenter', () => { if (!isMobile()) clearTimeout(hideTimeout); });
            showcase.addEventListener('mouseleave', () => { if (!isMobile()) hide(); });

            // Click behavior (mobile toggle, desktop toggle)
            link.addEventListener('click', (e) => {
                e.preventDefault();
                if (showcase.classList.contains('show')) hide();
                else show();
            });
            ...
            // Escape key
            document.addEventListener('keydown', (e) => {
                if (e.key === 'Escape' && showcase.classList.contains('show')) hide();
            });
            ...
            Utils.$$('.brand-card').forEach(card => {
                card.addEventListener('click', () => {
                    ...
                    if (brand) {
                        ...
                        Filters.updateTitle('brand', brand);
                        hide();
                        Utils.notify(`Mostrando productos ${card.querySelector('.brand-name')?.textContent || brand}`, 'info');
    ...
    const Newsletter = {
        init() {
            ...
            if (!shown) setTimeout(() => this.show(popup), CONFIG.delays.newsletterShow);
            if (closeBtn) closeBtn.addEventListener('click', () => this.hide(popup));
            popup.addEventListener('click', (e) => { if (e.target === popup) this.hide(popup); });
            document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && popup.classList.contains('active')) this.hide(popup); });
            if (form) form.addEventListener('submit', (e) => this.handleSubmit(e, popup));
        },
        show(popup) { popup.classList.add('active'); document.body.style.overflow = 'hidden'; },
        hide(popup) { popup.classList.remove('active'); document.body.style.overflow = ''; localStorage.setItem(CONFIG.storageKeys.newsletter, 'true'); },
        async handleSubmit(e, popup) {
            e.preventDefault();
            const email = Utils.$('#newsletterEmail')?.value.trim();
            if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { Utils.notify('Por favor, ingresa un email válido', 'error'); return; }
            const btn = Utils.$('.newsletter-submit-btn');
            if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Enviando...'; }
            await new Promise(resolve => setTimeout(resolve, 1500));
            const content = Utils.$('.newsletter-popup-content', popup);
            if (content) {
                content.innerHTML = `<div class="newsletter-popup-success active"><div class="newsletter-success-icon"><i class="fas fa-check"></i></div><h3 class="newsletter-success-title">¡Suscripción Exitosa!</h3><p class="newsletter-success-message">Gracias por suscribirte. Recibirás nuestras mejores ofertas en <strong>${email}</strong></p></div>`;
            }
            ...
            if (container.scrollTop > 300) {
                if (!this.button) {
                    ...
                    this.button.addEventListener('click', () => {
                        Utils.$('.products-scroll-container')?.scrollTo({ top: 0, behavior: 'smooth' });
                    });
    ...
    const ResponsiveHandler = {
        init() { window.addEventListener('resize', Utils.debounce(() => this.handleResize(), 250)); },
        handleResize() { if (window.innerWidth >= 992) Sidebar.close(); }
    };
    ...
    const App = {
        init() {
            ...
            Filters.updateProductCount();
            console.log('✅ JA Electrónica v2.1 - Sistema cargado');
        }
    };
